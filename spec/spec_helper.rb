...
RSpec.configure do |config|
  config.before(:suite) do
    shards_yml_path = File.join(File.dirname(__FILE__), "config/shards.yml")
    shards_yml = YAML.load(ERB.new(File.read(shards_yml_path)).result).with_indifferent_access
    shards_yml[:tako][:test].values.each do |conf|
      ActiveRecord::Tasks::DatabaseTasks.drop(conf)
      ActiveRecord::Tasks::DatabaseTasks.create(conf)
    end

    Tako.load_connections_from_yaml(shards_yml)
